...
// New: Distributions for dashboard charts
export const getTaskDistributionsQueryFn = async (
  workspaceId: string
): Promise<{
  message: string;
  ...
  const response = await API.get(`/task/workspace/${workspaceId}/distributions`);
  return response.data;
};

// New: Reports (CSV downloads)
export const downloadAllUsersTaskReport = async (workspaceId: string) => {
  const response = await API.get(`/task/workspace/${workspaceId}/report/all-tasks`, {
    responseType: "blob",
  });
  return response.data as Blob;
};

export const downloadDetailedTaskReport = async (
  workspaceId: string,
  params?: Partial<{
    projectId: string;
  ...
  const url = search.toString()
    ? `/task/workspace/${workspaceId}/report/detailed?${search.toString()}`
    : `/task/workspace/${workspaceId}/report/detailed`;
...
export const updateTaskStatusMutationFn = async (
  args: { workspaceId: string; taskId: string; status: string }
): Promise<{ message: string; task: any }> => {
  const response = await API.put(
    `/task/${args.taskId}/workspace/${args.workspaceId}/status`,
    { status: args.status }
  );
  return response.data;
};
...
export const addChecklistItemMutationFn = async (
  args: { workspaceId: string; taskId: string; title: string }
) => {
  const response = await API.post(
    `/task/${args.taskId}/workspace/${args.workspaceId}/checklist`,
    { title: args.title }
  );
  return response.data;
};
...
export const updateChecklistItemMutationFn = async (
  args: { workspaceId: string; taskId: string; itemId: string; patch: { title?: string; done?: boolean } }
) => {
  const response = await API.patch(
    `/task/${args.taskId}/workspace/${args.workspaceId}/checklist/${args.itemId}`,
    args.patch
  );
  return response.data;
};
...
export const deleteChecklistItemMutationFn = async (
  args: { workspaceId: string; taskId: string; itemId: string }
) => {
  const response = await API.delete(
    `/task/${args.taskId}/workspace/${args.workspaceId}/checklist/${args.itemId}`
  );
